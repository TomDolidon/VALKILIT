...
import static com.valkylit.domain.ReviewAsserts.*;
...
class ReviewResourceIT {
    ...
    public static Review createEntity() {
        return new Review().rating(DEFAULT_RATING).comment(DEFAULT_COMMENT);
    }
    ...
    public static Review createUpdatedEntity() {
        return new Review().rating(UPDATED_RATING).comment(UPDATED_COMMENT);
    }
    ...
    void putWithIdMismatchReview() throws Exception {
        ...
        restReviewMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(review))
            )
            .andExpect(status().isBadRequest());
