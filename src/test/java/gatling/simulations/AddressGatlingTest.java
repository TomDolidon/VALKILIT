...
public class AddressGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all addresses").get("/api/addresses").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created address").get("${new_address_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created address").delete("${new_address_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
