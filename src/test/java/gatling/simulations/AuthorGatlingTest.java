...
public class AuthorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all authors").get("/api/authors").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created author").get("${new_author_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created author").delete("${new_author_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
