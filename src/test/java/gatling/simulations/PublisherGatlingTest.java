...
public class PublisherGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all publishers").get("/api/publishers").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created publisher").get("${new_publisher_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created publisher").delete("${new_publisher_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
