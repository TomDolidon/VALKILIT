...
public class PurchaseCommandGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all purchaseCommands").get("/api/purchase-commands").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new purchaseCommand")
                        .post("/api/purchase-commands")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"expeditionDate\": \"2020-01-01T00:00:00.000Z\"" + ", \"status\": \"DRAFT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_purchaseCommand_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created purchaseCommand").get("${new_purchaseCommand_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created purchaseCommand").delete("${new_purchaseCommand_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PurchaseCommand entity").exec(scn);
