...
public class AwardBookGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all awardBooks").get("/api/award-books").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new awardBook")
                        .post("/api/award-books")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"year\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_awardBook_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created awardBook").get("${new_awardBook_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created awardBook").delete("${new_awardBook_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AwardBook entity").exec(scn);
